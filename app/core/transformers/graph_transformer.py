...
from app.core.transformers.nodes import generate_nodes
from app.core.transformers.edges import generate_edges
from app.core.transformers.pool_lanes import generate_pools_lanes
from app.core.transformers.transform_input_elements import normalize_flow_elements
...
class GraphTransformer:
    ...
    def visualize_graph_from_response(self, graph_response):
        """
        Mengubah hasil dari Neo4j (graph_response: list of {n, r, m})
        menjadi format nodes[] dan links[] untuk visualisasi.
        """
        nodes_map = {}
        links = []
        ...
        for record in graph_response:
            ...
            if n and n["id"] not in nodes_map:
                nodes_map[n["id"]] = {
                    ...
                    "type": n.get("type") or n.get("bpmn_type") or "node"
                }

            # Simpan node m
            if m and m["id"] not in nodes_map:
                nodes_map[m["id"]] = {
                    "id": m["id"],
                    "label": m.get("name") or m.get("id"),
                    "type": m.get("type") or m.get("bpmn_type") or "node"
                }
            ...
            if r and n and m:
                links.append({
                    ...
                    "label": r.get("name", "") or r.get("type", "RELATION")
                })
